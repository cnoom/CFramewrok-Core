...
namespace CFramework.Core.Editor.Utilities
{
    ...
    public abstract class EditorConfigUtility<T> : ScriptableObject where T : ScriptableObject
    {
        ...
        public static T LoadOrCreate()
        {
            ...
            var instance = CreateInstance<T>() as EditorConfigUtility<T>;
            var assetPath = instance?.DefaultAssetPath ?? string.Empty;
            ...
            var config = asset as EditorConfigUtility<T>;
        ...
        public static T LoadFromPath(string customPath)
        {
            ...
            var asset = AssetDatabase.LoadAssetAtPath<T>(customPath);
            if (!asset)
            {
                asset = CreateInstance<T>();
                var dir = Path.GetDirectoryName(customPath).Replace('\\', '/');
                if (!string.IsNullOrEmpty(dir))
                {
                    CFDirectoryUtility.EnsureFolder(dir);
                }
