...
namespace CFramework.Core.Editor.Utilities
{
    ...
    {
        private static readonly Dictionary<Type, ScriptableObject> EditorCache = new Dictionary<Type, ScriptableObject>();
        ...
        {
            Type type = typeof(T);
            if(EditorCache.TryGetValue(type, out ScriptableObject cached))
                return (T)cached;
            EditorConfigAttribute attr = type.GetCustomAttribute<EditorConfigAttribute>();
            if(attr == null)
            {
