...
namespace CFramework.Core.Utilities
{
    ...
    public class CFPool<TData> : IDisposable where TData : IPoolData
    {
        ...
        public CFPool(Func<TData> generate, Action<TData> get = null)
            : this(generate, get, onRelease: null, maxCapacity: int.MaxValue, prewarm: 0)
        {
        }
            ...
            int prewarm = 0)
        {
            ...
            var initialCapacity = Math.Min(maxCapacity, Math.Max(prewarm, 4));
            _pool = new Stack<TData>(initialCapacity);

            if (prewarm > 0)
            {
                var count = Math.Min(prewarm, _maxCapacity);
        ...
        public void Return(TData data)
        {
            ...
            if (!pushed && data is IDisposable disposable)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    UnityEngine.Debug.LogWarning($"[CFPool] 释放对象时发生异常: {ex.Message}\n堆栈: {ex.StackTrace}");
                }
            }
        }
        ...
        public void Prewarm(int count)
        {
            ...
            lock (_lock)
            {
                var canAdd = _maxCapacity - _pool.Count;
                if (canAdd <= 0) return;
                toCreate = Math.Min(count, canAdd);
            }

            var temp = new List<TData>(toCreate);
            for (int i = 0; i < toCreate; i++) temp.Add(_factory());

            lock (_lock)
            {
                foreach (var item in temp) _pool.Push(item);
            }
        }
        ...
        {
            get { lock (_lock) return _inUseCount; }
        }
