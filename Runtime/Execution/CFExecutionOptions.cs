...
namespace CFramework.Core.Execution
{
    ...
    {
        [InspectorName("继续执行")]
        Continue = 0, // 记录异常，继续执行其他处理器
        [InspectorName("遇错停止")]
        StopOnError = 1 // 首次异常即停止
    }

    public enum CancellationPolicy
    {
        [InspectorName("跳过当前")]
        SkipCurrent = 0, // 当前被取消则跳过该处理器，继续后续
        [InspectorName("取消全部")]
        CancelAll = 1 // 取消发生时，中止整条链（默认）
    }
    ...
    public class CFExecutionOptions
    {
        ...
        // 查询系统增强
        public bool QueryDeduplicateEnabled; // 是否对相同查询并发请求进行去重
        public bool QueryCacheEnabled; // 是否启用简单缓存
        public float QueryCacheTtlSeconds; // 缓存TTL（秒，<=0 表示不过期直至手动失效）

        // 可观测性（初级占位，后续可扩展为多 Sink/限流）
        public bool MetricsEnabled; // 简单指标计数是否启用
