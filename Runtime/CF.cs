...
namespace CFramework.Core
{
    ...
    public static class CF
    {
        ...
        /// </summary>
        private static readonly ConcurrentDictionary<object, CancellationTokenSource> _trackedCts = new ConcurrentDictionary<object, CancellationTokenSource>();
        ...
        private static CancellationToken LinkedToken(params CancellationToken[] ct)
        {
            ...
            CancellationToken[] validTokens = ct.Where(t => t != CancellationToken.None).ToArray();
            if(validTokens.Length == 0) return CancellationToken.None;
            if(validTokens.Length == 1) return validTokens[0];

            CancellationTokenSource cts = CancellationTokenSource.CreateLinkedTokenSource(validTokens);
            ...
            {
                cts.Dispose();
                if(_trackedCts.TryRemove(trackingKey, out CancellationTokenSource removed))
                {
        ...
        {
            foreach (KeyValuePair<object, CancellationTokenSource> kvp in _trackedCts)
            {
