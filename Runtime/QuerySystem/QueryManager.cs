...
namespace CFramework.Core.QuerySystem
{
    public class QueryManager
    {
        ...
        // 并发安全的处理器表
        private readonly ConcurrentDictionary<(Type, Type), QueryHandlerAsync> _dictionary = new ConcurrentDictionary<(Type, Type), QueryHandlerAsync>();
        ...
        // 去重与缓存（并发安全）
        private readonly ConcurrentDictionary<(Type queryType, Type resultType, int hash), object> _pending = new ConcurrentDictionary<(Type queryType, Type resultType, int hash), object>();

        private readonly CFPool<QueryHandlerAsync> _pool = new CFPool<QueryHandlerAsync>(() => new QueryHandlerAsync(), maxCapacity: 128, prewarm: 8);
        ...
        public async UniTask<(bool ok, TResult result)> TryQueryAsync<TQuery, TResult>(TQuery query,
            CancellationToken ct = default) where TQuery : IQueryData
        {
            ThrowIfDisposed();
            ...
            try
            {
                TResult res = await Query<TQuery, TResult>(query, ct);
        ...
        private static Delegate BuildFuncDelegate(object target, MethodInfo method)
        {
            ...
            Type delegateType = ReflectUtil.GetFuncType(method);
            ParameterInfo[] parameters = method.GetParameters();

            // 参数表达式
            ParameterExpression[] paramExprs = new ParameterExpression[parameters.Length];
            ...
            ConstantExpression instanceConst = Expression.Constant(target);
            Expression instanceExpr = method.IsStatic
                ? null
                : Expression.Convert(instanceConst, method.DeclaringType);

            // 调用表达式
            MethodCallExpression callExpr = method.IsStatic
        ...
        public void Register<T>(T queryObject) where T : class
        {
            ...
            {
                QueryHandlerAttribute attr = method.GetCustomAttribute<QueryHandlerAttribute>(true);
                if(attr == null) continue;

                ParameterInfo[] parameters = method.GetParameters();
                Type requestType = parameters.Length > 0 ? parameters[0].ParameterType : null;
                ...
                Type resultTypeArg = resultType.GetGenericArguments()[0];
        ...
        public void InvalidateCache<TQuery, TResult>(TQuery query) where TQuery : IQueryData
        {
            ThrowIfDisposed();
            EnsureOnMainThread();
            (Type, Type, int) key = MakeDedupKey(typeof(TQuery), typeof(TResult), query);
        ...
        public async UniTask DisposeAsync()
        {
            ...
            // 清空注册并归还池对象
            foreach (KeyValuePair<(Type, Type), QueryHandlerAsync> kv in _dictionary)
            {
                if(_dictionary.TryRemove(kv.Key, out QueryHandlerAsync handler))
                {
                    lock (_poolLock)
                    {
        ...
        private class ConcurrentLRUCache<TKey, TValue>
        {
            ...
            public bool TryGet(TKey key, out TValue value)
            {
                ...
                if(_valueCache.TryGetValue(key, out value))
                {
                    ...
                    try
                    {
                        if(_cache.TryGetValue(key, out LinkedListNode<TKey> node))
                        {
                            // 将访问的节点移到链表末尾（最新）
            ...
            public bool TryAdd(TKey key, TValue value)
            {
                ...
                try
                {
                    ...
                    if(_cache.TryGetValue(key, out LinkedListNode<TKey> existingNode))
                    {
                        _valueCache[key] = value;
                        // 移除旧节点，创建新节点并添加到末尾
                        _lruList.Remove(existingNode);
                        LinkedListNode<TKey> newNode = _lruList.AddLast(key);
                        _cache[key] = newNode;
                        return true;
                    }
                    ...
                    // 添加新项
                    LinkedListNode<TKey> newNode2 = _lruList.AddLast(key);
                    _cache[key] = newNode2;
                    _valueCache[key] = value;
                    return true;
                }
        ...
        private void Unsubscribe(Type queryType, Type resultType)
        {
            ...
            if(!_dictionary.TryRemove((queryType, resultType), out QueryHandlerAsync queryHandler)) return;
            lock (_poolLock)
            {
            ...
            where TQuery : IQueryData
        {
            ...
            if(_options?.QueryCacheEnabled == true)
            {
                ...
                {
                    double durCache = (DateTime.UtcNow - startTs).TotalMilliseconds;
                    _logger.LogDebug(
            ...
            {
                (Type, Type, int) key = MakeDedupKey(queryType, resultType, query);
                var created = false;

                object obj = _pending.GetOrAdd(key, _ =>
                {
                    created = true;
                    return InternalQueryAsync<TQuery, TResult>(query, context.CancellationToken);
                });

                UniTask<TResult> t = (UniTask<TResult>)obj;
                try
                {
                    TResult res = await t;
            ...
            {
                TResult res = await InternalQueryAsync<TQuery, TResult>(query, context.CancellationToken);
                TrySetCache(queryType, resultType, query, res);
                double dur = (DateTime.UtcNow - startTs).TotalMilliseconds;
                _logger.LogDebug(
        ...
        private bool TryGetCache<TResult>(Type queryType, Type resultType, object query, out TResult value)
        {
            ...
            (Type, Type, int) key = MakeDedupKey(queryType, resultType, query);
            if(_cache.TryGet(key, out (object value, DateTime time) entry))
            {
            ...
            where TQuery : IQueryData
        {
            Type queryType = typeof(TQuery);
            Type resultType = typeof(TResult);
            if(_dictionary.TryGetValue((queryType, resultType), out QueryHandlerAsync d))
            {
